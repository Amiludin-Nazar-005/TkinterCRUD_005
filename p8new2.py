...
def update_nilai(id_val, nama, bio, fis, ing, prediksi):
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute('''
...
class NilaiApp:
    def __init__(self, root):
        ...
        self.btn_update = ttk.Button(btn_frame, text='Submit Update', command=self.on_update, state='disabled')
        ...
        # Tombol untuk update dan delete di bawah treeview
        btn_tree_frame = ttk.Frame(frm_right)
        btn_tree_frame.grid(row=2, column=0, pady=8, sticky='w')
        self.btn_load_update = ttk.Button(btn_tree_frame, text='Update Selected', command=self.load_for_update)
        self.btn_load_update.grid(row=0, column=0, padx=(0, 8))
        self.btn_delete = ttk.Button(btn_tree_frame, text='Delete Selected', command=self.on_delete)
    ...
    def load_for_update(self):
        ...
        if not selected:
            messagebox.showwarning('Pilih Data', 'Pilih baris dari tabel untuk diupdate.')
            return
    ...
    def on_update(self):
        ...
        messagebox.showinfo('Sukses', f'Data diupdate. Prediksi: {prediksi}')
        self.clear_form()
    ...
    def on_delete(self):
        selected = self.tree.selection()
        if not selected:
            messagebox.showwarning('Pilih Data', 'Pilih baris dari tabel untuk dihapus.')
            return
        item = self.tree.item(selected[0])
    ...
    def export_csv(self):
        ...
        if not rows:
            ...
            return
        filename = f'nilai_siswa_export_{datetime.now().strftime("popet")}.csv'
